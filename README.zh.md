...
**给家长用的 12 周训练计划 —— 足球、棒球启蒙（T-ball）、腰旗橄榄球、篮球，适合 3–5 岁孩子。就算你自己完全没玩过这些运动，也能带。**
...
| **12 周赛季** | 四项运动。每周两次、每次 25 分钟，分四个阶段，从第一次碰球一直到真正的小型比赛。每周有一个重点和一个具体的观察目标。 |
| **56 个练习** | 每个练习都写清楚：怎么摆场地、怎么玩、该说的原话、以及现场怎么调难或调易。 |
...
3. 等一两分钟 —— 网站会出现在 `https://kidsbackyardcoach-cyber.github.io/`
...
© 2026 后院教练 Backyard Coach　版权所有。

这个项目公开是为了让家长能用上它，并不是开源软件。源码之所以可见，是因为它就是一个由 GitHub Pages 托管的 HTML 文件 —— 可见不等于授权。

**你可以：**自由地用在自己的孩子、球队或小团体上；分享链接；打印训练内容自用。

**以下情况请先联系我：**转发或自行架设副本、发布修改版本、把它打包进其他产品、或用于商业用途。

欢迎贡献内容和翻译 —— 见上文。提交即表示你同意这些内容可以被收录进本项目。如果你想在这个基础上做点什么，欢迎联系我，答案通常是可以。
